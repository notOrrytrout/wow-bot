...
pub fn check_spell_readiness(
    ...
    if !snapshot.state.session.in_world { return Err(SpellUnavailableReason::NotInWorld); }
    ...
    if snapshot.state.capabilities.spell_cooldowns.get(&spell).is_some_and(|ready_at| *ready_at > now_ms) {
...
mod tests {
    ...
    fn cooldown_and_zero_mana_are_shared_fail_closed_reasons() {
        let mut state=AuthoritativeState::default(); state.session.in_world=true; state.session.character_guid=Some(1);
        state.capabilities.spells.insert(686); state.capabilities.spell_cooldowns.insert(686,5000);
        state.entities.0.insert(EntityId(1),EntityState{id:EntityId(1),power_type:Some(0),power:Some((100,100)),..Default::default()});
        let snap=Snapshot::from_state(&state);
        assert_eq!(check_spell_readiness(&snap,686,None,1000),Err(SpellUnavailableReason::Cooldown));
        state.capabilities.spell_cooldowns.clear(); state.entities.0.get_mut(&EntityId(1)).unwrap().power=Some((0,100));
        assert_eq!(check_spell_readiness(&Snapshot::from_state(&state),686,None,1000),Err(SpellUnavailableReason::InsufficientPower));
    }
}
