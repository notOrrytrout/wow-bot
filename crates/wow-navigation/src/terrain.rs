...
pub enum TerrainKind { Ground, Water, Air, Unknown }
...
impl TerrainSampler {
    pub fn new(maps_dir: impl Into<PathBuf>) -> Result<Self, NavigationError> {
        ...
        if !maps_dir.is_dir() { return Err(NavigationError::MissingNavigationData); }
        Ok(Self { maps_dir, tiles: Arc::new(Mutex::new(HashMap::new())) })
    }

    pub fn maps_dir(&self) -> &Path { &self.maps_dir }
    ...
    fn tile(&self, path: PathBuf) -> Result<Arc<TerrainTile>, NavigationError> {
        if let Some(tile) = self.tiles.lock().map_err(|_| NavigationError::MissingNavigationData)?.get(&path).cloned() {
            return Ok(tile);
        }
        let bytes = fs::read(&path).map_err(|_| NavigationError::MissingNavigationData)?;
        let tile = Arc::new(TerrainTile::parse(&bytes)?);
        let mut cache = self.tiles.lock().map_err(|_| NavigationError::MissingNavigationData)?;
        if cache.len() >= 64 && !cache.contains_key(&path) {
            if let Some(old) = cache.keys().next().cloned() { cache.remove(&old); }
        }
...
pub(crate) fn map_tile_path(root: &Path, map: u32, x: f32, y: f32) -> Result<PathBuf, NavigationError> {
    if !x.is_finite() || !y.is_finite() { return Err(NavigationError::InvalidCoordinate); }
    let gx = (32.0 - x / GRID_SIZE) as i32;
    let gy = (32.0 - y / GRID_SIZE) as i32;
    if !(0..64).contains(&gx) || !(0..64).contains(&gy) { return Err(NavigationError::InvalidCoordinate); }
    Ok(root.join(format!("{map:03}{gx:02}{gy:02}.map")))
}

enum HeightGrid {
    Flat,
    U8 { v9: Vec<u8>, v8: Vec<u8>, scale: f32 },
    U16 { v9: Vec<u16>, v8: Vec<u16>, scale: f32 },
    Float { v9: Vec<f32>, v8: Vec<f32> },
}

struct TerrainTile { base: f32, heights: HeightGrid }

impl TerrainTile {
    fn parse(bytes: &[u8]) -> Result<Self, NavigationError> {
        if bytes.get(0..4) != Some(b"MAPS") || read_u32(bytes, 4)? != 9 { return Err(NavigationError::MissingNavigationData); }
        let offset = read_u32(bytes, 20)? as usize;
        if offset == 0 || bytes.get(offset..offset + 4) != Some(b"MHGT") { return Err(NavigationError::MissingNavigationData); }
        let flags = read_u32(bytes, offset + 4)?;
        let base = read_f32(bytes, offset + 8)?;
        let max = read_f32(bytes, offset + 12)?;
        if !base.is_finite() || !max.is_finite() { return Err(NavigationError::InvalidCoordinate); }
        let data = offset + 16;
    ...
    fn height(&self, world_x: f32, world_y: f32) -> Result<f32, NavigationError> {
        ...
        x -= xi as f32; y -= yi as f32;
        let row = (xi & 127) as usize; let col = (yi & 127) as usize;
        let v9 = row * 129 + col; let v8 = row * 128 + col;
        let value = match &self.heights {
            HeightGrid::Flat => self.base,
            HeightGrid::U8 { v9: a, v8: b, scale } => interpolate(a[v9] as f32, a[v9 + 129] as f32, a[v9 + 1] as f32, a[v9 + 130] as f32, b[v8] as f32, x, y) * *scale + self.base,
            HeightGrid::U16 { v9: a, v8: b, scale } => interpolate(a[v9] as f32, a[v9 + 129] as f32, a[v9 + 1] as f32, a[v9 + 130] as f32, b[v8] as f32, x, y) * *scale + self.base,
            HeightGrid::Float { v9: a, v8: b } => interpolate(a[v9], a[v9 + 129], a[v9 + 1], a[v9 + 130], b[v8], x, y),
        };
        value.is_finite().then_some(value).ok_or(NavigationError::InvalidCoordinate)
    }
}

fn interpolate(h1: f32, h2: f32, h3: f32, h4: f32, h5: f32, x: f32, y: f32) -> f32 {
    let (a, b, c) = if x + y < 1.0 {
        if x > y { (h2 - h1, 2.0 * h5 - h2 - h1, h1) } else { (2.0 * h5 - h3 - h1, h3 - h1, h1) }
    } else if x > y {
...
fn read_slice(bytes: &[u8], offset: usize, len: usize) -> Result<&[u8], NavigationError> {
    bytes.get(offset..offset.checked_add(len).ok_or(NavigationError::MissingNavigationData)?).ok_or(NavigationError::MissingNavigationData)
}
fn read_u32(bytes: &[u8], offset: usize) -> Result<u32, NavigationError> {
    let b = read_slice(bytes, offset, 4)?; Ok(u32::from_le_bytes([b[0], b[1], b[2], b[3]]))
}
fn read_f32(bytes: &[u8], offset: usize) -> Result<f32, NavigationError> { Ok(f32::from_bits(read_u32(bytes, offset)?)) }
fn read_u16_vec(bytes: &[u8], offset: usize, count: usize) -> Result<Vec<u16>, NavigationError> {
    let raw = read_slice(bytes, offset, count.checked_mul(2).ok_or(NavigationError::MissingNavigationData)?)?;
...
fn read_f32_vec(bytes: &[u8], offset: usize, count: usize) -> Result<Vec<f32>, NavigationError> {
    let raw = read_slice(bytes, offset, count.checked_mul(4).ok_or(NavigationError::MissingNavigationData)?)?;
    let values: Vec<f32> = raw.chunks_exact(4).map(|b| f32::from_le_bytes([b[0], b[1], b[2], b[3]])).collect();
    if values.iter().any(|v| !v.is_finite()) { return Err(NavigationError::InvalidCoordinate); }
    Ok(values)
}
