...
pub fn validate(config: &RuntimeConfig) -> Result<(), String> {
    if config.worker_queue == 0 {
        return Err("worker_queue must be positive".into());
    }
    if config.handshake_timeout_ms == 0 {
        return Err("handshake_timeout_ms must be positive".into());
    }
    if config.control_bind.trim().is_empty() {
        return Err("control_bind is required".into());
    }
    config
        .control_bind
        .parse::<std::net::SocketAddr>()
        .map_err(|e| format!("invalid control_bind: {e}"))?;
    Ok(())
}
...
fn validate_data_directory(kind: &str, path: &Path) -> Result<(), String> {
    ...
    if !md.is_dir() {
        return Err(format!(
            "required {kind} data path is not a directory: {}",
            path.display()
        ));
    }
    let mut entries = fs::read_dir(path).map_err(|e| {
        format!(
            "failed to read required {kind} data at {}: {e}",
            path.display()
        )
    })?;
    if entries.next().is_none() {
        return Err(format!(
            "required {kind} data directory is empty: {}",
            path.display()
        ));
    }
    Ok(())
