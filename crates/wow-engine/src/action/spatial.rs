...
pub fn profile(command: &GameplayCommand) -> Option<SpatialProfile> {
    let (target, maximum_range, approach_range, facing_tolerance, requires_line_of_sight) =
        match command {
            GameplayCommand::Attack(target) => {
                (*target, 5.0, 4.0, Some(MELEE_FACING_TOLERANCE), true)
            }
            ...
            | GameplayCommand::EnterVehicle(target) => {
                (*target, 5.0, 4.0, Some(INTERACTION_FACING_TOLERANCE), true)
            }
            ...
            | GameplayCommand::VehicleCast {
                target: Some(target),
                ..
            } => (*target, 20.0, 18.0, Some(CAST_FACING_TOLERANCE), true),
