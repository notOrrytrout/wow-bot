...
quest target. Quest item-use rules require a named quest item, an item use spell
in `item_template`, and one live-spawn target named in the use clause. If the
source data does not identify one target, the generator records the candidate
under `unresolved_quest_action_hints`; it does not turn that candidate into a
runtime action.
...
live entity and the engine still validates the action before execution.
