...
use tokio::{io::{AsyncBufReadExt, BufReader}, net::{TcpListener, TcpStream}, process::{Child, Command}, sync::{mpsc, Mutex}};
...
use wow_infra::config::{app::{AccountConfig, AppConfig}, data_dir::AppPaths};
...
#[derive(Clone)]
struct Args { config: Option<PathBuf>, worker_bin: Option<PathBuf>, data_root: Option<PathBuf> }
fn args() -> Result<Args> {
    let mut config = None;
    ...
    while let Some(arg) = it.next() {
        match arg.as_str() {
            "--config" => config = Some(PathBuf::from(it.next().context("--config requires a path")?)),
            "--worker-bin" => worker_bin = Some(PathBuf::from(it.next().context("--worker-bin requires a path")?)),
            "--data-root" => data_root = Some(PathBuf::from(it.next().context("--data-root requires a path")?)),
            "--help" | "-h" => {
                println!("usage: wow-bot-supervisor [--config path] [--worker-bin path] [--data-root path]");
    ...
    Ok(Args { config, worker_bin, data_root })
}
...
async fn main() -> Result<()> {
    ...
    let config_path = args.config.clone().unwrap_or_else(|| app_paths.config.clone());
    let (mut config, created) = AppConfig::load_or_create(&config_path).map_err(anyhow::Error::msg)?;
    ...
    let listener = TcpListener::bind(&config.runtime.control_bind).await
        .with_context(|| format!("bind worker control listener {}", config.runtime.control_bind))?;
    tracing::info!(bind=%config.runtime.control_bind, "worker control listener ready");

    let (supervisor_tx, mut supervisor_rx) = mpsc::channel::<SupervisorCommand>(config.runtime.worker_queue);
    ...
    for account in config.accounts.iter().filter(|a| a.enabled) {
        ...
        lane_io.insert(account.lane, Arc::new(LaneIo {
        ...
        managed.push(ManagedLane {
            config: ProxyAccountConfig {
                account: account.account, lane: account.lane, worker: generation,
                account_name: account.account_name.clone(), password: account.password.clone(),
                character: (!account.character.trim().is_empty()).then(|| account.character.clone()),
            },
            worker_rx: w2p_rx,
            worker_tx: p2w_tx,
    ...
    if managed.is_empty() { bail!("no enabled accounts are configured"); }
    ...
    for account in config.accounts.iter().filter(|a| a.enabled) {
        children.push(spawn_worker(&worker_bin, account.lane, WorkerGeneration(1), &config.runtime.control_bind, &runtime_paths.maps).await?);
    }
        ...
        while let Some(command) = supervisor_rx.recv().await {
            match &command {
                SupervisorCommand::Shutdown => {
                    for route in dispatch_routes.values() { let _ = route.command_tx.send(command.clone()).await; }
                    break;
                }
                SupervisorCommand::StartLane(lane) | SupervisorCommand::StopLane(lane)
                | SupervisorCommand::SetPause { lane, .. } | SupervisorCommand::UpdatePause { lane, .. }
                | SupervisorCommand::ReplaceMission { lane, .. } => {
                    if let Some(route) = dispatch_routes.get(lane) { let _ = route.command_tx.send(command).await; }
                }
            }
        }
    });
    ...
    let proxy = ProxyRuntimeConfig {
        auth_bind: config.proxy.auth_bind.clone(), world_bind: config.proxy.world_bind.clone(),
        transparent_world_bind: config.proxy.transparent_world_bind.clone(), advertise_host: config.proxy.advertise_host.clone(),
        upstream_auth_host: config.upstream.auth_host.clone(), upstream_auth_port: config.upstream.auth_port,
        upstream_world_host: config.upstream.world_host.clone(), upstream_world_port: config.upstream.world_port,
    ...
    for lane in routes.keys().copied() { supervisor_tx.send(SupervisorCommand::StartLane(lane)).await.ok(); }
    ...
    dispatcher.abort();
    for child in &mut children { terminate(child).await; }
    Ok(())
}
...
fn prepare_runtime_asset_manifest(config: &AppConfig, app_paths: &AppPaths) -> Result<()> {
    ...
    let manifest = wow_infra::world_knowledge::generate::generate_runtime_asset_manifest(&resolved, &output)
        .with_context(|| format!("build runtime asset manifest from {}", config.runtime.runtime_data.root.display()))?;
    tracing::info!(
        path=%output.display(),
...
fn ensure_runtime_data(config: &mut AppConfig, config_path: &Path, explicit_root: Option<&Path>) -> Result<()> {
    if let Some(root) = explicit_root {
        let root = detect_runtime_data_root(root)
            .with_context(|| format!("--data-root does not contain valid AzerothCore runtime data: {}", root.display()))?;
    ...
    loop {
        ...
        let selected = if io::stdin().is_terminal() {
            ...
            println!("You may paste/type the path now, or press Enter to open the native folder browser.");
...
POSIX path of chosenFolder"#,
    ...
    if output.status.success() {
        let value = String::from_utf8(output.stdout).context("macOS folder chooser returned invalid UTF-8")?;
...
fn pick_runtime_data_folder(start_dir: Option<&Path>) -> Result<Option<PathBuf>> {
    ...
    if let Some(start_dir) = start_dir { dialog = dialog.set_directory(start_dir); }
...
fn detect_runtime_data_root(selected: &Path) -> Option<PathBuf> {
    ...
    if selected.file_name().and_then(|name| name.to_str()).is_some_and(|name| {
        matches!(name.to_ascii_lowercase().as_str(), "dbc" | "maps" | "vmaps" | "mmaps")
    }) {
        if let Some(parent) = selected.parent() { candidates.push(parent.to_path_buf()); }
    }
...
fn persist_runtime_data_root(config: &mut AppConfig, config_path: &Path, root: PathBuf) -> Result<()> {
...
fn picker_start_directory(configured: &Path) -> Option<PathBuf> {
    if configured.is_dir() { return Some(configured.to_path_buf()); }
    configured.parent().filter(|parent| parent.is_dir()).map(Path::to_path_buf)
}


fn ensure_interactive_setup(config: &mut AppConfig, config_path: &Path, created: bool) -> Result<()> {
    ...
    if !needs_setup { return Ok(()); }

    if !io::stdin().is_terminal() {
        if needs_account {
            bail!("no enabled bot account is configured and standard input is not interactive; run the supervisor in a terminal once to complete setup, or provide a prepared --config file");
        }
        return Ok(());
    }

    println!("\n=== wow-bot first-run setup ===");
    println!("AzerothCore runtime data is treated as read-only. wow-bot writes only to <repo>/wow-bot-data (or WOW_BOT_HOME when explicitly set).\n");
...
fn configure_network_topology(config: &mut AppConfig) -> Result<()> {
    ...
    let remote_clients = prompt_yes_no("Will WoW clients on other computers connect to this bot server?", false)?;
    if remote_clients {
        let advertised = detect_reachable_local_ip().unwrap_or_else(|| {
            tracing::warn!("could not automatically determine a reachable LAN address for this machine");
            String::new()
        });
        let advertised = if advertised.is_empty() {
            prompt_required("Hostname or LAN IP that those WoW clients can reach")?
        } else {
            println!("Detected this bot server at {advertised}; remote WoW clients will be directed here.");
            advertised
        };
...
async fn check_upstream_services(config: &AppConfig) -> Result<()> {
    ...
    let auth = format!("{}:{}", config.upstream.auth_host, config.upstream.auth_port);
    let world = format!("{}:{}", config.upstream.world_host, config.upstream.world_port);
    ...
    let same_machine = config.upstream.auth_host == "127.0.0.1"
        && config.upstream.world_host == "127.0.0.1";
...
async fn probe_upstream(label: &str, endpoint: &str, timeout: Duration) -> std::result::Result<(), String> {
    match tokio::time::timeout(timeout, TcpStream::connect(endpoint)).await {
        ...
        Err(_) => Err(format!("{label} did not respond at {endpoint} within {} seconds", timeout.as_secs())),
    }
}
...
fn configure_first_account(config: &mut AppConfig) -> Result<()> {
    ...
    let password = loop {
        ...
        if !value.is_empty() { break value; }
        println!("Password cannot be empty.");
    };
    let character = prompt_optional("Character name (optional)")?;

    let next_lane = config.accounts.iter().map(|a| a.lane.get()).max().unwrap_or(0) + 1;
    let next_account = config.accounts.iter().map(|a| a.account.get()).max().unwrap_or(0) + 1;
...
fn prompt_yes_no(label: &str, default: bool) -> Result<bool> {
    ...
    loop {
        ...
        if value.is_empty() { return Ok(default); }
        match value.as_str() {
...
async fn accept_workers(listener: TcpListener, routes: Arc<HashMap<LaneId, Arc<LaneIo>>>) -> Result<()> {
    loop {
        let (stream, peer) = listener.accept().await?;
        let routes = routes.clone();
        tokio::spawn(async move {
            if let Err(e) = attach_worker(stream, routes).await { tracing::warn!(%peer, error=%format_args!("{e:#}"), "worker control connection ended"); }
        });
    }
}
...
async fn attach_worker(stream: TcpStream, routes: Arc<HashMap<LaneId, Arc<LaneIo>>>) -> Result<()> {
    ...
    let (lane, generation) = match hello { WorkerWire::Hello { lane, generation } => (lane, generation), _ => bail!("first worker frame was not Hello") };
    let route = routes.get(&lane).with_context(|| format!("worker connected for unknown lane {lane}"))?.clone();
    if route.generation != generation { bail!("stale worker generation {} for lane {}", generation.get(), lane); }
    let mut p2w = route.proxy_to_worker.lock().await.take().context("lane already has a worker connection")?;
    let mut commands = route.commands.lock().await.take().context("lane command receiver already attached")?;
    ...
    loop {
        match read_frame::<_, WorkerWire>(&mut reader).await {
            Ok(WorkerWire::Proxy(message)) => { if w2p.send(message).await.is_err() { break; } }
            Ok(WorkerWire::Event(event)) => log_worker_event(event),
            Ok(WorkerWire::Hello { .. }) => bail!("worker sent duplicate Hello"),
            Err(e) => { writer_task.abort(); return Err(e.into()); }
        }
    }
    writer_task.abort();
    Ok(())
...
fn log_worker_event(event: WorkerEvent) {
    match event {
        WorkerEvent::Ready { lane, generation } => tracing::info!(%lane, generation=generation.get(), "worker ready"),
        WorkerEvent::Heartbeat { lane, generation } => tracing::trace!(%lane, generation=generation.get(), "worker heartbeat"),
        WorkerEvent::Failed { lane, generation, reason } => tracing::error!(%lane, generation=generation.get(), %reason, "worker failed"),
        WorkerEvent::Stopped { lane, generation } => tracing::info!(%lane, generation=generation.get(), "worker stopped"),
    }
}

async fn spawn_worker(path: &Path, lane: LaneId, generation: WorkerGeneration, control: &str, maps_dir: &Path) -> Result<Child> {
    tracing::info!(%lane, worker=%path.display(), "starting worker process");
    let mut child = Command::new(path)
        .arg("--lane").arg(lane.to_string())
        .arg("--generation").arg(generation.get().to_string())
        .arg("--control").arg(control)
        .arg("--maps-dir").arg(maps_dir)
        .stdin(Stdio::null()).stdout(Stdio::piped()).stderr(Stdio::piped())
        .spawn().with_context(|| format!("start worker {}", path.display()))?;
...
async fn resolve_worker_bin(explicit: Option<PathBuf>) -> Result<PathBuf> {
    ...
    let worker_name = if cfg!(windows) { "wow-bot-worker.exe" } else { "wow-bot-worker" };
    ...
    let workspace_root = env::current_dir().ok().and_then(|cwd| find_workspace_root(&cwd));
    ...
    if let Some(root) = workspace_root.as_ref() {
        ...
        command.current_dir(root).arg("build").arg("-p").arg("wow-bot-worker");
        if profile == "release" { command.arg("--release"); }
        let status = command.status().await.context("run cargo to build wow-bot-worker")?;
        if !status.success() {
            bail!("automatic worker build failed with status {status}; run `cargo build -p wow-bot-worker{}` in {}", if profile == "release" { " --release" } else { "" }, root.display());
        }
    ...
    bail!("wow-bot-worker was not found. Searched: {searched}. Run `cargo build -p wow-bot-worker`, install the worker beside the supervisor, or pass --worker-bin <path>")
}
...
fn find_workspace_root(start: &Path) -> Option<PathBuf> {
    for candidate in start.ancestors() {
        ...
        if text.contains("[workspace]") && text.contains("apps/wow-supervisor") && text.contains("apps/wow-worker") {
...
async fn terminate(child: &mut Child) { let _ = child.start_kill(); let _ = child.wait().await; }
